...
- (void)makedegreed:(CGPoint)location and:(CGPoint)prevLocation
{
    ...
    CGFloat nx = location.x - circleHeart.x; //象限正负的判断
    CGFloat ny = location.y - circleHeart.y;
    CGFloat px = prevLocation.x - circleHeart.x;
    CGFloat py = prevLocation.y - circleHeart.y;
    int cxiang = 0, dxiang = 0; //cxiang 是最新点
    ...
    //3.2 在同一个象限的情况 和不同象限的情况
    CGFloat nxx = location.x - prevLocation.x; // 2个点之间 差  X 差  nx - px
    CGFloat nyy = location.y - prevLocation.y; // Y 差

    if (cxiang == dxiang) { //单纯在一个象限的情况
//        if (nyy > 0) { //nx>px 的情况
//            //
//            if (cxiang == 4 || cxiang == 3) {
//                degreed = -degreed; //角度变成负的   逆时针变 负数
//            }
//        }
//        else if (nyy < 0) { //nx<px
//            if (cxiang == 1 || cxiang == 2) {
//                degreed = -degreed; //逆时针 变负数
//            }
//        } else{  ///x相同,看Y的变化
//            if (nxx > 0) {
//                degreed = -degreed; //逆时针 变负数
//
//            }
//            NSLog(@"  nxiangxian:%d  pxiangxian:%d  degreed:%f",cxiang,dxiang,degreed);
//
//        }
        
        switch (cxiang) {
            case 1:
                if ((nxx<0 && nyy <0 )) {//||(nxx>0 && nyy<0)
                    degreed = -degreed;
                } else if ((nxx>0 && nyy > 0) ){//|| (nxx> 0 && nyy<0)
                    degreed = degreed;
                }else if( nyy < 0 || nxx < 0){
                    degreed = -degreed;

                }
                NSLog(@" 1111 nxx:%f  nyy:%f degreed:%f",nxx,nyy,degreed);
            ...
            case 2:
                ...
                NSLog(@" 2222 nxx:%f  nyy:%f degreed:%f",nxx,nyy,degreed);
            ...
            case 3:
                ...
                NSLog(@" 3333 nxx:%f  nyy:%f degreed:%f",nxx,nyy,degreed);
            ...
            case 4:
                ...
                NSLog(@" 444 nxx:%f  nyy:%f degreed:%f",nxx,nyy,degreed);
                break;
            default:
                break;
        }
    }else if (cxiang < dxiang) { //逆时针时针  //n象限是cxiang  p象限是dxiang
        if (cxiang == 1 && dxiang == 4) {
            NSLog(@"顺时针 nxiangxian:%d  pxiangxian:%d",cxiang,dxiang);

        } else {
            degreed = - degreed;
            NSLog(@" 逆时针 nxiangxian:%d  pxiangxian:%d",cxiang,dxiang);
        }

    }else if (cxiang > dxiang) {
        if (cxiang == 4 && dxiang == 1) {
            degreed = - degreed;
            NSLog(@" 逆时针 nxiangxian:%d  pxiangxian:%d",cxiang,dxiang);
